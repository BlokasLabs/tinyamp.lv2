...
#   make CFLAGS=-O2
#   make install DESTDIR=$(CURDIR)/debian/tinygain.lv2 PREFIX=/usr
#
...
tinygain_VERSION?=$(shell git describe --tags HEAD 2>/dev/null | sed 's/-g.*$$//;s/^v//' || echo "LV2")
###############################################################################
LIB_EXT=.so
...
ifneq ($(MOD),)
  ...
  MODLABELMO=mod:label \"TinyGain Mono\";
  MODLABELST=mod:label \"TinyGain Stereo\";
...
$(BUILDDIR)$(LV2NAME).ttl: lv2ttl/$(LV2NAME).ttl.in
	@mkdir -p $(BUILDDIR)
	sed "s/@LV2NAME@/$(LV2NAME)/;s/@SIGNATURE@/$(SIGNATURE)/;s/@VERSION@/lv2:microVersion $(LV2MIC) ;lv2:minorVersion $(LV2MIN) ;/g;s/@MODBRAND@/$(MODBRAND)/;s/@MODLABELMONO@/$(MODLABELMO)/;s/@MODLABELSTEREO@/$(MODLABELST)/" \
